...
async function buildState(userId) {
    ...
    if (row && row.characterId && row.character) {
      ...
      character = {
        ...
        milestones: HERO_MILESTONES.map((target,index) => ({
          ...
          effect: target===10?`Production ×2 et passif : ${RARITY_PASSIVE[row.character.rarity]||'bonus personnel'}`:target===500?'Production ×2 et Ascension débloquée':'Production ×2 supplémentaire',
        })),
  ...
  return {
    ...
    battle: {
      ...
      skills:{burstReadyAt:user.idleBurstReadyAt?.toISOString()||null,teamReadyAt:user.idleTeamReadyAt?.toISOString()||null,burstDamage:burstPreview,teamDamage:teamPreview,uniqueRoles:uniqueActiveRoles,teamWindowSeconds:20+uniqueActiveRoles*5},
    },
    missions,
