...
const IDLE_ROLES = {
  ...
  assassin:{ name: 'Assassin', icon: 'fa-bolt', color: '#ffd54a', description:'+3% DPS · +25% sous 20% PV' },
...
function renderIdleTeamStrategy(state) {
  ...
  const presets=document.getElementById('idle-presets');if(presets){const squads=state.strategy?.squads?.slots||[];presets.innerHTML=squads.length?squads.map((squad)=>`<article class="idle-squad-slot ${squad.unlocked?'unlocked':'locked'} ${squad.saved?'saved':''}"><header><i class="fas ${escapeHtml(squad.icon||'fa-users')}"></i><span><small>${squad.unlocked?'SLOT DISPONIBLE':`BLOQUÉ · ${escapeHtml(squad.unlock?.label||'objectif requis')}`}</small><b>${escapeHtml(squad.name)}</b><em>${escapeHtml(squad.purpose||'Composition')}</em></span></header><p>${escapeHtml(squad.bonus||'')}</p><footer><button class="btn-secondary" data-preset-save-slot="${squad.index}" ${squad.unlocked?'':'disabled'}><i class="fas fa-floppy-disk"></i> Sauver ici</button><button class="btn-primary" data-preset-load="${escapeHtml(squad.name)}" ${squad.saved&&squad.unlocked?'':'disabled'}><i class="fas fa-play"></i> Charger${squad.saved?` · ${squad.size}/10`:''}</button></footer></article>`).join(''):`<p class="hint">Aucune squad disponible.</p>`;}
}
