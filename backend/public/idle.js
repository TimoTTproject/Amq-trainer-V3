...
let idleFavoriteItems = (()=>{try{return new Set(JSON.parse(localStorage.getItem('idle-favorite-items')||'[]'))}catch{return new Set()}})();
let idleCompetitiveTab=localStorage.getItem('idle-competitive-tab')||'optimizer';
let idleCompetitiveSimulation={levels:0,gear:0,formation:'current'};
...
// `requireText` : protection renforcée façon « écris RECYCLER » — le bouton de
// confirmation reste grisé tant que le texte exact n'est pas saisi (la saisie
// navigateur native souffre du même mutisme WebView que confirm().)
function idleConfirm(message,{title='Confirmer ?',confirmLabel='Continuer',cancelLabel='Annuler',requireText=null}={}){
  return new Promise((resolve)=>{
    ...
    overlay.innerHTML=`<div class="modal-card idle-confirm-card"><h3 id="idle-confirm-title"><i class="fas fa-circle-question"></i> ${escapeHtml(title)}</h3><p>${escapeHtml(message)}</p>${requireText?`<label class="idle-confirm-guard"><small>Écris <b>${escapeHtml(requireText)}</b> pour confirmer</small><input type="text" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="${escapeHtml(requireText)}"></label>`:''}<div class="modal-actions"><button type="button" class="btn-secondary" data-confirm="no">${escapeHtml(cancelLabel)}</button><button type="button" class="btn-primary" data-confirm="yes" ${requireText?'disabled':''}>${escapeHtml(confirmLabel)}</button></div></div>`;
    const done=(value)=>{overlay.remove();resolve(value);};
    overlay.addEventListener('click',(e)=>{const button=e.target.closest('[data-confirm]');if(button&&!button.disabled)return done(button.dataset.confirm==='yes');if(e.target===overlay)done(false);});
    if(requireText){
      const input=overlay.querySelector('.idle-confirm-guard input');const yes=overlay.querySelector('[data-confirm="yes"]');
      input?.addEventListener('input',()=>{if(yes)yes.disabled=input.value.trim()!==requireText;});
      input?.addEventListener('keydown',(e)=>{if(e.key==='Enter'&&input.value.trim()===requireText)done(true);});
    }
    document.body.appendChild(overlay);
    (requireText?overlay.querySelector('.idle-confirm-guard input'):overlay.querySelector('[data-confirm="yes"]'))?.focus();
  });
}
...
function persistIdleFavoriteItems(){localStorage.setItem('idle-favorite-items',JSON.stringify([...idleFavoriteItems]));}
...
function idleStopTicker() {
  ...
  idleOrbTimer = null;
  clearInterval(idleRuneDungeonTimer);
  idleRuneDungeonTimer = null;
  idleRuneDungeonPendingHits = 0;
...
function renderIdleState(state) {
  ...
  if (idleActivePanel === 'activities') {
    renderIdleLaunchReward(state.launchReward);
  ...
  if (idleActivePanel === 'team') {
    renderIdleTeamStrategy(state);
    renderIdleTeamComparison(state);
  ...
  renderIdleUpgradeShortcuts(state);
...
function idleCompetitiveActions(state){
  const rate=Math.max(1,Number(state.totalRate||0));const actions=[];
  const add=(key,label,cost,gain,tab,detail,icon='fa-arrow-trend-up')=>{cost=Number(cost||0);gain=Math.max(0,Number(gain||0));if(!cost||!gain)return;actions.push({key,label,cost,gain,tab,detail,icon,roi:cost/gain,percent:gain/rate*100,affordable:Number(state.essence||0)>=cost});};
  if(state.prod?.nextCost&&state.prod?.nextMultiplier)add('prod','Discipline',state.prod.nextCost,rate*(state.prod.nextMultiplier/state.prod.multiplier-1),'upgrades','Augmente toute la production','fa-chart-line');
  if(state.crit?.nextCost)add('crit','Précision',state.crit.nextCost,rate*Math.max(0,(state.crit.nextChance-state.crit.chance)*.75),'upgrades','Renforce le rendement moyen des critiques','fa-crosshairs');
  if(state.multiStrike?.nextCost)add('multiStrike','Multi-frappe',state.multiStrike.nextCost,rate*Math.max(0,(state.multiStrike.nextBonus-state.multiStrike.bonus)),'upgrades','Gain direct sur les frappes et le burst','fa-hand-fist');
  for(const item of state.inventory?.items||[]){if(!item.equipped||item.enhancementLevel>=15)continue;const gain=rate*Math.max(.001,Number(item.effectiveBonus||item.bonus||0)*.065);add(`item:${item.id}`,`${item.name} +${(item.enhancementLevel||0)+1}`,item.enhanceCost,gain,'equipment',`${item.equippedCharacter||'Héros'} · ${item.setName||'Set'}`,'fa-shield-halved');}
  for(const slot of state.slots||[]){if(!slot.character)continue;const empty=(slot.character.equipments||[]).filter((item)=>item.empty).length;if(empty)actions.push({key:`gear:${slot.index}`,label:`Équiper ${slot.character.name}`,cost:0,gain:rate*.035*empty,tab:'equipment',detail:`${empty} emplacement${empty>1?'s':''} vide${empty>1?'s':''}`,icon:'fa-wand-magic-sparkles',roi:0,percent:3.5*empty,affordable:true,free:true});}
  return actions.sort((a,b)=>Number(b.free)-Number(a.free)||a.roi-b.roi||b.percent-a.percent);
}
function idleCompetitiveDominatedItems(state){
  const items=(state.inventory?.items||[]).filter((item)=>!item.equipped&&!item.locked&&!idleFavoriteItems.has(item.id));
  return items.filter((item)=>items.some((other)=>other.id!==item.id&&other.kind===item.kind&&other.setKey===item.setKey&&Number(other.effectiveBonus||0)>Number(item.effectiveBonus||0)*1.12&&(other.enhancementLevel||0)<=(item.enhancementLevel||0)));
}
function idleCompetitivePanel(state,tab){
  if(state.rift&&state.rift.floor==null)state.rift.floor=state.rift.bestFloor;
  const rate=Math.max(0,Number(state.totalRate||0));const actions=idleCompetitiveActions(state);const top=actions.slice(0,5);const battle=state.battle||{};const mechanic=battle.mechanic;const efficiency=state.dojo?.prestige?.efficiency||{};const runs=efficiency.runs||[];
  if(tab==='optimizer')return `<div class="idle-competitive-summary"><article class="hero"><small>MEILLEUR LEVIER</small><b>${escapeHtml(top[0]?.label||'Aucune optimisation urgente')}</b><strong>${top[0]?`+${top[0].percent.toFixed(1)}% estimé`:'Build stabilisé'}</strong><p>${escapeHtml(top[0]?.detail||'Prépare le prochain Prestige ou vise un record.')}</p></article><article><small>DPS ACTUEL</small><b>${idleFormatNumber(rate)}/s</b><em>${idleFormatNumber(state.battle?.skills?.burstDamage||0)} burst</em></article><article><small>ESSENCE</small><b>${idleFormatNumber(state.essence)}</b><em>${top.filter((x)=>x.affordable).length} action(s) rentable(s)</em></article></div><div class="idle-competitive-action-list">${top.map((item,index)=>`<button data-comp-goto="${item.tab}" ${item.key.startsWith('gear:')?`data-comp-slot="${item.key.split(':')[1]}"`:''} class="${index?'':'best'}"><i class="fas ${item.icon}"></i><span><small>${index?'OPTION '+(index+1):'FAIS CECI MAINTENANT'}</small><b>${escapeHtml(item.label)}</b><em>${escapeHtml(item.detail)}</em></span><strong>+${item.percent.toFixed(1)}%<small>${item.free?'GRATUIT':`${idleFormatNumber(item.cost)} · ${idleFormatDuration(item.roi*1000)}`}</small></strong></button>`).join('')||idleEmptyState('fa-circle-check','Build optimisé','Aucun achat ou emplacement vide détecté.')}</div>`;
  if(tab==='boss'){
    const roleAdvice={shield:'Les frappeurs rapides brisent le bouclier.',ward:'Conserve le Combo d’équipe pour dissiper la garde.',regen:'Garde l’Ultime pour couper la régénération.',focus:'Frappe la faille puis déclenche l’Ultime.',rage:'Maximise le burst avant l’enrage.',counter:'Espace les compétences et privilégie le DPS continu.'};const required=state.combat?.analysis?.requiredDps||0;const margin=rate-required;
    return `<div class="idle-boss-prep ${margin>=0?'ready':'blocked'}"><i class="fas ${mechanic?IDLE_BOSS_MECHANIC_ICONS[mechanic.key]||'fa-skull':'fa-chess-knight'}"></i><span><small>PLAN DU PROCHAIN GARDIEN</small><b>${escapeHtml(mechanic?.name||battle.world?.enemyName||'Gardien en approche')}</b><p>${escapeHtml(mechanic?.description||'Prépare un burst complet et une composition équilibrée.')}</p><em>${escapeHtml(roleAdvice[mechanic?.key]||'Assassin pour le burst · Support pour les recharges · Tank pour la stabilité.')}</em></span><strong>${required?`${margin>=0?'+':'−'}${idleFormatNumber(Math.abs(margin))} DPS`:'ANALYSE EN COURS'}<small>${margin>=0?'MARGE ESTIMÉE':'DÉFICIT À COMBLER'}</small></strong></div><div class="idle-competitive-buttons"><button data-comp-goto="team"><i class="fas fa-users-gear"></i> Ajuster l’équipe</button><button data-comp-goto="home"><i class="fas fa-crosshairs"></i> Ouvrir le combat</button></div>`;
  }
  if(tab==='loadouts'){const squads=state.strategy?.squads?.slots||[];return `<p class="idle-comp-note"><i class="fas fa-circle-info"></i> Un loadout restaure maintenant l’équipe, le chef, la formation, les six objets de chaque héros, la vitesse, le mode et les compétences automatiques.</p><div class="idle-comp-loadouts">${squads.map((s)=>`<article class="${s.saved?'saved':''} ${s.unlocked?'':'locked'}"><i class="fas ${s.icon}"></i><span><small>${s.unlocked?(s.saved?'PROFIL COMPLET':'EMPLACEMENT VIDE'):escapeHtml(s.unlock?.label||'Verrouillé')}</small><b>${escapeHtml(s.name)}</b><em>${escapeHtml(s.purpose)}${s.saved?` · ${s.size} héros`:''}</em></span><button data-comp-preset-save="${s.index}" ${s.unlocked?'':'disabled'}><i class="fas fa-floppy-disk"></i></button>${s.saved?`<button class="primary" data-comp-preset-load="${escapeHtml(s.name)}"><i class="fas fa-play"></i></button>`:''}</article>`).join('')}</div>`;}
  if(tab==='simulator'){
    const formation=idleCompetitiveSimulation.formation==='current'?state.strategy?.formations?.find((f)=>f.active):state.strategy?.formations?.find((f)=>f.key===idleCompetitiveSimulation.formation);const currentFormation=state.strategy?.formations?.find((f)=>f.active);const formationFactor=(formation?.multiplier||1)/(currentFormation?.multiplier||1);const projected=rate*(1+idleCompetitiveSimulation.levels*.012)*(1+idleCompetitiveSimulation.gear*.01)*formationFactor;const gain=projected-rate;
    return `<div class="idle-simulator-result"><span><small>AVANT</small><b>${idleFormatNumber(rate)} DPS</b></span><i class="fas fa-arrow-right"></i><span class="after"><small>APRÈS SIMULATION</small><b>${idleFormatNumber(projected)} DPS</b><em>${gain>=0?'+':''}${idleFormatNumber(gain)} · ${rate?`${(gain/rate*100).toFixed(1)}%`:'—'}</em></span></div><div class="idle-simulator-controls"><label>Niveaux de héros <span><button data-comp-sim="levels:-5">−5</button><b>${idleCompetitiveSimulation.levels>=0?'+':''}${idleCompetitiveSimulation.levels}</b><button data-comp-sim="levels:5">+5</button></span></label><label>Niveaux d’objets <span><button data-comp-sim="gear:-3">−3</button><b>${idleCompetitiveSimulation.gear>=0?'+':''}${idleCompetitiveSimulation.gear}</b><button data-comp-sim="gear:3">+3</button></span></label><label>Formation <select data-comp-sim-formation><option value="current">Actuelle</option>${(state.strategy?.formations||[]).map((f)=>`<option value="${f.key}" ${idleCompetitiveSimulation.formation===f.key?'selected':''}>${escapeHtml(f.name)}${f.conditionMet?'':' · inactive'}</option>`).join('')}</select></label></div><p class="idle-comp-note"><i class="fas fa-flask"></i> Projection indicative : aucun achat ni changement n’est appliqué.</p>`;
  }
  if(tab==='endgame'){const season=state.season||{};const next=(season.tiers||[]).find((tier)=>!tier.completed);return `<div class="idle-endgame-grid"><article><i class="fas fa-ranking-star"></i><small>CLASSEMENTS SPÉCIALISÉS</small><b>Mesure la performance réelle</b><p>DPS de fin de run, rendement de Sagesse, Faille et vitesse de Prestige.</p><span><button data-comp-ranking="dps">DPS</button><button data-comp-ranking="efficiency">RENDEMENT</button><button data-comp-ranking="rift">FAILLE</button></span></article><article><i class="fas fa-calendar-days"></i><small>${escapeHtml(season.name||'SAISON EN COURS')}</small><b>${idleFormatNumber(season.level||0)} points</b><p>${next?`Prochaine récompense à ${idleFormatNumber(next.level)} points`:'Tous les paliers sont atteints.'}</p><button data-comp-goto="activities">Voir la saison</button></article><article><i class="fas fa-dungeon"></i><small>CHALLENGE HEBDOMADAIRE</small><b>Faille ${idleFormatNumber(state.rift?.floor||0)}/20</b><p>${escapeHtml(state.run?.weeklyEvent?.name||'Mutateur actif cette semaine')}</p><button data-comp-goto="farm">Préparer la Faille</button></article></div>`;}
  if(tab==='inventory'){const dominated=idleCompetitiveDominatedItems(state);const free=(state.inventory?.items||[]).filter((x)=>!x.equipped);const locked=free.filter((x)=>x.locked||idleFavoriteItems.has(x.id));return `<div class="idle-inventory-audit"><article><small>SANS RISQUE DÉTECTÉ</small><b>${dominated.length}</b><em>objets dominés par un meilleur jet du même set</em></article><article><small>PROTÉGÉS / FAVORIS</small><b>${locked.length}</b><em>exclus de la sélection automatique</em></article><article><small>PLACE LIBRE</small><b>${Math.max(0,(state.inventory?.capacity||120)-(state.inventory?.count||0))}</b><em>sur ${state.inventory?.capacity||120} emplacements</em></article></div><div class="idle-competitive-buttons"><button data-comp-select-dominated ${dominated.length?'':'disabled'}><i class="fas fa-filter-circle-dollar"></i> Examiner les ${dominated.length} objets dominés</button><button data-comp-goto="equipment"><i class="fas fa-box-open"></i> Ouvrir l’inventaire</button></div>`;}
  if(tab==='prestige'){const current=Number(efficiency.currentWisdomPerHour||0);const avg=Number(efficiency.averageWisdomPerHour||0);const recommendation=efficiency.recommendation==='prestige'?'Prestige maintenant':'Continue à pousser';return `<div class="idle-prestige-competitive"><article class="decision"><small>DÉCISION RECOMMANDÉE</small><b>${recommendation}</b><p>${efficiency.recommendation==='prestige'?'Ton rendement est au niveau de ta moyenne ou meilleur. Sécurise la Sagesse.':'Le gain horaire reste sous ta moyenne historique : quelques stages supplémentaires seront plus efficaces.'}</p><button data-comp-goto="upgrades"><i class="fas fa-brain"></i> Voir le Prestige</button></article><article><small>RUN ACTUELLE</small><b>${current.toFixed(2)} Sagesse/h</b><em>${avg?`${current>=avg?'+':''}${((current/avg-1)*100).toFixed(0)}% vs moyenne`:'Première mesure'}</em></article><article><small>ORIENTATION PERMANENTE</small><b>${escapeHtml(state.ancients?.focus?.name||'Non choisie')}</b><em>${escapeHtml(state.ancients?.focus?.description||'Investis ta Sagesse dans une voie spécialisée.')}</em></article></div>`;}
  const best=runs.reduce((max,run)=>Math.max(max,Number(run.teamDps||0)),0);const latest=runs[0];const trend=runs.length>1&&runs[1].teamDps?((latest.teamDps/runs[1].teamDps-1)*100):null;return `<div class="idle-record-overview"><article><small>RECORD DPS DE RUN</small><b>${idleFormatNumber(best)}/s</b><em>${trend==null?'Une seconde run affichera la tendance':`${trend>=0?'+':''}${trend.toFixed(1)}% sur la dernière run`}</em></article><article><small>RUNS ARCHIVÉES</small><b>${runs.length}</b><em>${idleFormatNumber(efficiency.total||runs.length)} Prestiges mesurés</em></article><button data-comp-share-build><i class="fas fa-share-nodes"></i><span><b>Partager mon build</b><small>Copie l’équipe, la formation, le DPS et le Prestige.</small></span></button></div><div class="idle-record-mini">${runs.slice(0,5).map((run,index)=>`<span><strong>#${index+1}</strong><b>Stage ${idleFormatNumber(run.bestStage)}</b><em>${idleFormatNumber(run.teamDps)} DPS · ${formatMs(run.durationMs)}</em></span>`).join('')||'<p class="hint">Termine un Prestige pour créer ton premier rapport.</p>'}</div>`;
}
function renderIdleCompetitiveCenter(state){const content=document.getElementById('idle-competitive-content');if(!content)return;const valid=['optimizer','boss','loadouts','simulator','endgame','inventory','prestige','records'];if(!valid.includes(idleCompetitiveTab))idleCompetitiveTab='optimizer';document.querySelectorAll('[data-competitive-tab]').forEach((button)=>button.classList.toggle('active',button.dataset.competitiveTab===idleCompetitiveTab));const score=document.getElementById('idle-competitive-score');if(score)score.textContent=`SCORE ${idleFormatNumber(Math.round(Math.log10(Math.max(10,state.totalRate||0))*1000+(state.battle?.bestStage||1)*10+(state.dojo?.prestige?.level||0)*250))}`;content.innerHTML=idleCompetitivePanel(state,idleCompetitiveTab);}
async function shareIdleCompetitiveBuild(){const state=idleState;if(!state)return;const heroes=(state.slots||[]).filter((slot)=>slot.character).map((slot)=>slot.character.name).join(', ');const formation=state.strategy?.formations?.find((f)=>f.active)?.name||'Libre';const text=`Anime Ascension · Build compétitif\n${idleFormatNumber(state.totalRate)} DPS · Stage ${idleFormatNumber(state.battle?.bestStage||1)} · Prestige ${idleFormatNumber(state.dojo?.prestige?.level||0)}\n${formation} · ${heroes}`;try{await navigator.clipboard.writeText(text);idleNotify('Build compétitif copié.','success');}catch{idleNotify('Impossible de copier le build.','error');}}

function renderIdleAdvisor(advisor,roadmap=[]) {
  const box=document.getElementById('idle-advisor');const focus=document.getElementById('idle-focus-objective');if(!box&&!focus)return;
  if(!advisor){box?.classList.add('hidden');if(box)box.innerHTML='';focus?.classList.add('hidden');if(focus)focus.innerHTML='';return;}
  const steps=(roadmap||[]).slice(0,3);
  if(box){box.className=`idle-advisor priority-${escapeHtml(advisor.priority||'normal')}`;
  ...
  if(focus){const current=steps[0];focus.className=`idle-focus-objective priority-${escapeHtml(advisor.priority||'normal')}`;focus.innerHTML=`<i class="fas ${escapeHtml(advisor.icon||'fa-compass')}"></i><span><small>PROCHAINE ACTION${current?' · ÉTAPE 1/'+steps.length:''}</small><b>${escapeHtml(current?.title||advisor.title)}</b><em>${escapeHtml(current?.action||advisor.description)}</em></span><button type="button" data-advisor-tab="${escapeHtml(current?.tab||advisor.tab||'home')}" data-advisor-command="${escapeHtml(current?.command||advisor.command||'')}">${escapeHtml(current?.action||advisor.action||'Continuer')} <i class="fas fa-arrow-right"></i></button>`;}
}
...
function renderIdleTeamComparison(state){
  const box=document.getElementById('idle-team-compare');if(!box)return;const active=(state.slots||[]).filter((slot)=>slot.character);
  if(!active.length){box.innerHTML='<p class="hint"><i class="fas fa-user-plus"></i> Ajoute un héros pour commencer la comparaison.</p>';return;}
  const total=Math.max(1,Number(state.totalRate||0));const rows=active.map((slot)=>{const c=slot.character;const gear=(state.inventory?.items||[]).filter((item)=>item.equippedSlotIndex===slot.index);const empty=Math.max(0,6-gear.length);const enhanced=gear.length?Math.round(gear.reduce((sum,item)=>sum+Number(item.enhancementLevel||0),0)/gear.length):0;const activeSets=idleItemSetStatuses(gear).filter((set)=>set.stacks>0).length;return {slot,c,role:idleRoleFor(c),rate:Number(c.rate||c.baseRate||0),share:Math.round(Number(c.rate||c.baseRate||0)/total*100),gear:gear.length,empty,enhanced,activeSets};});
  const lever=[...rows].sort((a,b)=>b.empty-a.empty||a.activeSets-b.activeSets||a.rate-b.rate)[0];const leaderId=state.strategy?.leaderCharacterId;
  box.innerHTML=`<header><span><small>LECTURE RAPIDE</small><h3>Comparer les contributions</h3></span><em><i class="fas fa-wand-magic-sparkles"></i> Priorité : ${escapeHtml(lever.c.name)} · ${lever.empty?`${lever.empty} emplacement${lever.empty>1?'s':''} vide${lever.empty>1?'s':''}`:lever.activeSets?'améliorer les objets':'compléter un set'}</em></header><div>${rows.map((row)=>`<button type="button" class="${row===lever?'priority':''}" data-team-compare-slot="${row.slot.index}" style="--role:${row.role.color}"><span class="idle-team-compare-portrait" ${row.c.imageUrl?`style="background-image:url('${escapeHtml(row.c.imageUrl)}')"`:''}></span><span><small>${row.c.id===leaderId?'<i class="fas fa-crown"></i> CHEF · ':''}${escapeHtml(row.role.name).toUpperCase()}</small><b>${escapeHtml(row.c.name)}</b><em>${row.share}% du DPS total</em></span><strong>${idleFormatNumber(row.rate)}<small>/s</small></strong><span class="idle-team-gear-score ${row.empty?'incomplete':'complete'}"><i class="fas fa-shield-halved"></i><b>${row.gear}/6</b><small>${row.activeSets} set${row.activeSets>1?'s':''} · moy. +${row.enhanced}</small></span></button>`).join('')}</div>`;
}
...
async function runIdleRuneDungeon(kind){
  ...
  try{
    const result=await api('/api/idle/rune-dungeon/start',{method:'POST',body:JSON.stringify({kind})});idleState=result.state;renderIdleState(result.state);idleSpawnRuneDungeonFloat('DESCENTE LANCÉE','xp');
...
function idleSpawnRuneDungeonFloat(text,cls='damage'){
  const box=document.querySelector('.idle-rune-dungeon-floats');if(!box)return;const float=document.createElement('span');float.className=cls;float.textContent=text;float.style.setProperty('--float-x',`${42+Math.round(Math.random()*16)}%`);box.appendChild(float);setTimeout(()=>float.remove(),900);
}
async function flushIdleRuneDungeonHits(){
  ...
  try{const before=idleState.runeDungeon.floor||0;const result=await api('/api/idle/rune-dungeon/hit',{method:'POST',body:JSON.stringify({count,requestId:`rune-${Date.now()}-${Math.random().toString(36).slice(2,10)}`})});if(result.duplicate)return;idleState=result.state;renderIdleState(result.state);if(result.damage)idleSpawnRuneDungeonFloat(`-${idleFormatNumber(result.damage)}`,result.criticals?'crit':'damage');if(result.floorCleared&&!result.cleared){const clearedCount=Math.max(1,result.floorsCleared||1);idleSpawnRuneDungeonFloat(clearedCount>1?`${clearedCount} ÉTAGES NETTOYÉS`:`ÉTAGE ${result.floor} NETTOYÉ`,'xp');document.querySelector('.idle-rune-dungeon-arena')?.classList.add('floor-cleared');}if(result.cleared){showIdleRuneDungeonReward(result);if(typeof sfx!=='undefined'&&sfx.idleChest)sfx.idleChest();}else if((result.state?.runeDungeon?.floor||0)>before&&typeof sfx!=='undefined'&&sfx.idleHit)sfx.idleHit(true);}catch(e){idleNotify(e.message,'error');}finally{idleRuneDungeonBusy=false;}
}
...
function renderIdleInventory(state){
  ...
  items=[...items].sort((a,b)=>Number(idleFavoriteItems.has(b.id))-Number(idleFavoriteItems.has(a.id))||(idleItemSort==='recent'?new Date(b.obtainedAt)-new Date(a.obtainedAt):idleItemSort==='rarity'?(IDLE_RARITY_ORDER[b.rarity]-IDLE_RARITY_ORDER[a.rarity]):((b.effectiveBonus||b.bonus)-(a.effectiveBonus||a.bonus))));
  ...
  grid.querySelectorAll('[data-item-id]').forEach((card)=>{const item=inventory.items.find((entry)=>String(entry.id)===card.dataset.itemId);if(!item)return;card.classList.add(`kind-${item.kind}`);card.classList.toggle('favorite',idleFavoriteItems.has(item.id));const actions=card.querySelector('.idle-item-card-actions');if(actions&&!actions.querySelector('[data-item-favorite]'))actions.insertAdjacentHTML('afterbegin',`<button type="button" data-item-favorite="${escapeHtml(item.id)}" title="${idleFavoriteItems.has(item.id)?'Retirer des favoris':'Ajouter aux favoris'}" aria-pressed="${idleFavoriteItems.has(item.id)}"><i class="fas fa-star"></i></button>`);});
...
// Objet légendaire/mythique dans la sélection : UNE seule modale, avec la
// saisie « RECYCLER » intégrée (l'ancienne saisie navigateur séparée est muette dans les
// WebViews mobiles — le recyclage semblait alors simplement annulé).
async function salvageIdleItems(ids){const items=(idleState?.inventory?.items||[]).filter((item)=>ids.includes(item.id));if(!items.length)return;const total=items.reduce((sum,item)=>sum+(item.salvageValue||0),0);const precious=items.some((item)=>['legendary','mythic'].includes(item.rarity));const accepted=await idleConfirm(`Recycler ${items.length} objet${items.length>1?'s':''} contre environ ${idleFormatNumber(total)} Essence ?${precious?' ATTENTION : la sélection contient un objet légendaire ou mythique.':''}`,{title:'Recycler la sélection ?',confirmLabel:'Recycler',requireText:precious?'RECYCLER':null});if(!accepted){if(precious)idleNotify('Objet précieux conservé : confirmation annulée.','success');return;}try{const r=await api('/api/idle/equipment/salvage',{method:'POST',body:JSON.stringify({ids,confirmHighRarity:precious?'RECYCLER':false})});idleSelectedItems.clear();renderIdleState(r.state);idleSpawnFloat(`RECYCLAGE +${idleFormatNumber(r.gained)}`,'xp');}catch(e){idleNotify(e.message,'error');}}
...
function renderIdleBattle(battle, dojo, prevBattle) {
  ...
  // En Farm, le bandeau porte la sortie du mode à UN clic : le mode s'active
  // parfois implicitement (voyage rapide) et la moitié des retours joueurs
  // venaient de joueurs coincés en Farm sans le savoir.
  if(objective){objective.classList.toggle('is-farming',farming);objective.innerHTML=`<i class="fas ${farming?'fa-coins':boss?'fa-crown':'fa-forward-step'}"></i><span><b>${farming?`MODE FARM · ${boss?'le boss réapparaît après chaque victoire':`la vague ${wave} recommence après le dernier ennemi`}`:boss?'Vague 10/10 · Boss final':`Vague ${wave}/10 · ${enemiesRemaining} ennemi${enemiesRemaining>1?'s':''} restant${enemiesRemaining>1?'s':''}`}</b><small>${farming?boss?'Chaque victoire compte (missions, défis) mais ne fait pas progresser':'La progression est volontairement figée sur cette vague':boss?'Vaincs-le pour passer au monde suivant':`${escapeHtml(battle?.enemy?.name||'Standard')} · ${escapeHtml(battle?.enemy?.description||'')} · +${idleFormatNumber(battle?.reward||0)} Essence`}</small></span>${farming?'<button type="button" data-idle-farm-exit><i class="fas fa-forward"></i> Repasser en Progression</button>':`<strong>${boss?'Monde suivant':`Puis vague ${wave+1}`}</strong>`}`;}
  if (waveTrack) {
...
async function chooseIdleFormation(formation){try{renderIdleState(await api('/api/idle/formation',{method:'POST',body:JSON.stringify({formation})}));refreshIdleStrategyLabIfOpen();}catch(e){idleNotify(e.message,'error');}}
...
async function saveIdlePreset(slotIndex=null){const squad=(idleState?.strategy?.squads?.slots||[]).find((slot)=>slot.index===Number(slotIndex));const name=squad?.name||document.getElementById('idle-preset-name')?.value.trim();if(!name)return;try{renderIdleState(await api('/api/idle/team-preset/save',{method:'POST',body:JSON.stringify({name,slotIndex:squad?.index})}));idleAddCombatLog(`Squad ${name} sauvegardée`,'fa-floppy-disk');refreshIdleStrategyLabIfOpen();}catch(e){idleNotify(e.message,'error');}}
async function loadIdlePreset(name){try{renderIdleState(await api('/api/idle/team-preset/load',{method:'POST',body:JSON.stringify({name})}));idleAddCombatLog(`Preset ${name} chargé`,'fa-users-gear');refreshIdleStrategyLabIfOpen();}catch(e){idleNotify(e.message,'error');}}
async function claimIdleChallenge(key){try{const r=await api('/api/idle/challenge/claim',{method:'POST',body:JSON.stringify({key})});idleSpawnFloat(`DÉFI +${r.reward} SCEAUX`,'crit');renderIdleState(r.state);}catch(e){idleNotify(e.message,'error');}}
...
async function chooseIdleHeroStyle(type,key) { try { const state=await api('/api/idle/hero-style',{method:'POST',body:JSON.stringify({type,key})}); idleState=state; openIdleClassPicker(); renderIdleState(state); } catch(e){idleNotify(e.message,'error');} }
async function chooseIdleHeroSpec(key) { try { const state=await api('/api/idle/hero-specialization',{method:'POST',body:JSON.stringify({key})}); idleState=state; openIdleClassPicker(); renderIdleState(state); } catch(e){idleNotify(e.message,'error');} }
...
function idleLootCardHTML(loot){
  if(!loot)return '';
  const kindLabel=IDLE_ITEM_META[loot.kind]?.label||'Objet';
  const target=(idleState?.slots||[]).find((slot)=>slot.index===idleEquipmentTargetSlot&&slot.character);
  const comparison=loot.stored&&target?`<div class="idle-loot-comparison"><small>APERÇU SUR ${escapeHtml(target.character.name).toUpperCase()}</small>${idleItemComparison(loot,idleEquipmentTargetSlot)}</div>`:'';
  return `<div class="idle-boss-loot ${escapeHtml(loot.rarity)}">${idleItemArt(loot,'reveal')}<div><small>${escapeHtml(idleRarityLabel(loot.rarity).toUpperCase())} · SET ${escapeHtml(loot.setKey||'energy').toUpperCase()}</small><b>${escapeHtml(loot.name||kindLabel)}</b><span>${loot.stored?`Objet ajouté · ${escapeHtml(kindLabel)} · +0`:`Inventaire plein · converti en +${idleFormatNumber(loot.salvage||0)} Essence`}</span></div><em>${loot.stored?'NOUVEAU':'RECYCLÉ'}</em></div>${comparison}${loot.stored?'<button class="btn-secondary idle-boss-open-items" data-open-equipment><i class="fas fa-diamond"></i> Voir et équiper l’objet</button>':''}`;
}
function applyIdleRewardRarity(modal,rarity){
  modal.classList.remove('reward-rare','reward-epic','reward-legendary','reward-mythic');
  modal.classList.add(`reward-${rarity||'rare'}`);
}
function showIdleBossReward(reward) {
  ...
  applyIdleRewardRarity(modal,reward.loot?.rarity);
  modal.classList.remove('hidden');
}
...
function showIdleRuneDungeonReward(result){
  ...
  applyIdleRewardRarity(modal,result.loot?.rarity);
  modal.classList.remove('hidden');
}
...
function renderIdleMissions(missions,rank) {
  ...
  box.innerHTML = missions.length ? missions.map((m) => {const progress=Math.min(100,m.progress/Math.max(1,m.target)*100);const boost=m.rewardBoost?.active?`<em class="return-boost-pill"><i class="fas fa-bolt"></i> ×${m.rewardBoost.multiplier}</em>`:'';return `<article class="idle-mission ${m.completed ? 'done' : ''}"><span class="idle-mission-icon"><i class="fas ${m.completed?'fa-check':m.cadence === 'Quotidienne' ? 'fa-sun' : 'fa-calendar-week'}"></i></span><div><small>${escapeHtml(m.cadence)} · ${m.completed?'TERMINÉE':'EN COURS'} ${boost}</small><b>${escapeHtml(m.title)}</b><span>${escapeHtml(m.description)}</span><div class="idle-mission-progress"><em style="--progress:${progress}%"></em><strong>${idleFormatNumber(Math.min(m.progress,m.target))} / ${idleFormatNumber(m.target)}</strong></div></div><button class="btn-secondary" data-idle-mission="${m.key}" ${!m.completed || m.claimed ? 'disabled' : ''}>${m.claimed ? '<i class="fas fa-check"></i> Réclamée' : `${idleRewardLabel(m)}<small>Réclamer</small>`}</button></article>`;}).join('') : idleEmptyState('fa-list-check','Aucune mission pour le moment','Reviens après ta prochaine synchronisation — de nouvelles missions arrivent chaque jour et chaque semaine.');
...
function renderIdleLaunchReward(launch){
  const box=document.getElementById('idle-launch-reward');if(!box)return;
  if(!launch){box.classList.add('hidden');return;}box.classList.remove('hidden');
  const reward=launch.reward||{};box.className=`idle-launch-reward ${launch.claimed?'claimed':''}`;
  box.innerHTML=`<div class="idle-launch-vfx" aria-hidden="true"><i></i><i></i><i></i></div><header><span class="idle-launch-version">VERSION <b>2.0</b></span><small>ÉVÉNEMENT DE RETOUR · CADEAU EXCEPTIONNEL</small><h3>${launch.claimed?'Bienvenue dans la nouvelle ère':'Anime Ascension revient en force'}</h3><p>${launch.claimed?'Ton pack Retour 2.0 est sécurisé sur ton compte. La nouvelle saison peut commencer.':'Un nouveau départ mérite mieux qu’un simple bonus : ouvre ton pack Retour 2.0 offert.'}</p></header><div class="idle-launch-loot"><article><i class="fas fa-fire-flame-curved"></i><span><small>ESSENCE</small><b>+${idleFormatNumber(reward.essence||0)}</b><em>Accélération immédiate</em></span></article><article><i class="fas fa-ticket"></i><span><small>SCEAUX</small><b>+${idleFormatNumber(reward.seals||0)}</b><em>${idleFormatNumber(reward.seals||0)} invocations</em></span></article><article><i class="fas fa-coins"></i><span><small>TOKENS GACHA</small><b>+${idleFormatNumber(reward.tokens||0)}</b><em>Pour célébrer le retour</em></span></article></div><button type="button" class="idle-launch-claim" data-idle-launch-claim ${launch.claimed?'disabled':''}>${launch.claimed?'<i class="fas fa-circle-check"></i><span><small>PACK OUVERT</small><b>Récompenses réclamées</b></span>':'<i class="fas fa-gift"></i><span><small>OFFERT · UNE FOIS PAR COMPTE</small><b>Ouvrir le pack Retour 2.0</b></span><i class="fas fa-chevron-right"></i>'}</button>`;
}

function showIdleLaunchRewardModal(reward,edition){
  document.getElementById('idle-launch-celebration')?.remove();const modal=document.createElement('div');modal.id='idle-launch-celebration';modal.className='modal-overlay idle-launch-celebration';modal.setAttribute('role','dialog');modal.setAttribute('aria-modal','true');modal.setAttribute('aria-label','Pack Retour 2.0 ouvert');
  modal.innerHTML=`<div class="idle-launch-celebration-card"><div class="idle-launch-rays"></div><button type="button" class="modal-close" data-launch-close aria-label="Fermer"><i class="fas fa-times"></i></button><span class="idle-launch-celebration-badge">VERSION ${idleFormatNumber(edition||2)}.0</span><i class="fas fa-gift idle-launch-celebration-icon"></i><small>PACK RETOUR OUVERT</small><h2>Des récompenses exceptionnelles !</h2><p>Merci d’être là pour cette nouvelle saison d’Anime Ascension.</p><div><article><i class="fas fa-fire-flame-curved"></i><b>+${idleFormatNumber(reward.essence||0)}</b><span>Essence</span></article><article><i class="fas fa-ticket"></i><b>+${idleFormatNumber(reward.seals||0)}</b><span>Sceaux</span></article><article><i class="fas fa-coins"></i><b>+${idleFormatNumber(reward.tokens||0)}</b><span>Tokens</span></article></div><button type="button" class="btn-primary" data-launch-close>Commencer la saison <i class="fas fa-arrow-right"></i></button></div>`;
  modal.addEventListener('click',(e)=>{if(e.target===modal||e.target.closest('[data-launch-close]'))modal.remove();});document.body.appendChild(modal);requestAnimationFrame(()=>modal.classList.add('visible'));
}
...
async function claimIdleLaunchReward(){try{const r=await api('/api/idle/launch-reward/claim',{method:'POST',body:'{}'});const reward=r.reward||{};idleSpawnFloat(`VERSION ${r.edition}.0 · PACK OUVERT`,'crit huge');showIdleLaunchRewardModal(reward,r.edition);if(typeof burstConfetti==='function')burstConfetti(140);await refreshIdleState();}catch(e){idleNotify(e.message,'error');}}
function openIdleGuide(){document.getElementById('idle-guide-modal')?.classList.remove('hidden');}
function idleRankingMetric(type,value){if(type==='speed')return idleFormatDuration(Number(value)||0)||'—';if(type==='efficiency')return `${Number(value||0).toFixed(2)}/h`;if(type==='dps')return `${idleFormatNumber(value)}/s`;return idleFormatNumber(value);}
function idleRankingRows(players,type){
  ...
  const metricLabel=type==='speed'?'CHRONO':type==='dps'?'DPS':type==='efficiency'?'SAGESSE/H':type==='collection'?'HÉROS':type==='rift'?'PALIER':type==='level'?'RANG':'VAGUE';
...
async function openIdleRanking(type=idleRankingType){
  ...
  const titles={stage:'Vague',dps:'DPS',efficiency:'Sagesse/h',level:'Niveau',speed:'Temps',rift:'Palier',collection:'Héros',friends:'Vague'};if(metric)metric.textContent=titles[type]||'Score';
  ...
  const hints={
    ...
    dps:'Meilleur DPS enregistré à la fin d’une run de Prestige. Optimise tes objets, ta formation et tes synergies.',
    efficiency:'Meilleur rendement de Sagesse par heure sur une run terminée. Trouve le bon moment pour Prestiger.',
...
async function claimIdleAchievement(key) { try { const r = await api('/api/idle/achievement/claim', { method: 'POST', body: JSON.stringify({ key }) }); idleSpawnFloat(`SUCCÈS +${idleFormatNumber(r.reward)}`, 'crit'); if(r.tokens)idleNotify(`+${idleFormatNumber(r.tokens)} tokens gacha gagnés !`,'success'); if (typeof burstConfetti === 'function') burstConfetti(25); await refreshIdleState(); } catch(e){idleNotify(e.message,'error');} }
async function claimIdleEvent() { try { const r = await api('/api/idle/event/claim', { method: 'POST', body: JSON.stringify({}) }); idleSpawnFloat(`CONVERGENCE +${idleFormatNumber(r.reward)}`, 'crit'); await refreshIdleState(); } catch(e){idleNotify(e.message,'error');} }
...
// Raccourcis de l'écran COMBAT alimentés par les données de l'onglet
// Améliorer : rendus à CHAQUE synchro (pas seulement depuis Améliorer), sinon
// « Ascension disponible » et le pulse du bouton Sagesse restaient figés tant
// qu'on ne visitait pas l'onglet — même famille de bug que les bénédictions
// (cf. test idle-panel-render-audit).
function renderIdleUpgradeShortcuts(state){
  const prestige=state.dojo?.prestige;
  const quick=document.getElementById('idle-prestige-quick');
  if(quick&&prestige){
    const stageReady=prestige.runBestStage>=prestige.minStage;const remainingMinutes=Math.ceil(Math.max(0,(prestige.minRunMs||0)-(prestige.runElapsedMs||0))/60000);
    quick.classList.toggle('available',!!prestige.eligible);
    quick.innerHTML=`<i class="fas fa-brain"></i><span><small>${prestige.eligible?'ASCENSION DISPONIBLE':'PROCHAINE ASCENSION'}</small><b>+${idleFormatNumber(prestige.reward)} Sagesse</b><em>${prestige.eligible?'Voir et confirmer':stageReady?`Stage validé · encore ${remainingMinutes} min`:'Record '+idleFormatNumber(prestige.runBestStage)+' / '+idleFormatNumber(prestige.minStage)}</em></span><i class="fas fa-arrow-right"></i>`;
  }
  const ancients=state.ancients;
  if(ancients)document.getElementById('idle-spend-wisdom')?.classList.toggle('idle-affordable',ancients.items.some((it)=>it.unlocked&&ancients.points>=it.cost));
}
...
function initIdleUI() {
  ...
  document.getElementById('idle-next-objective')?.addEventListener('click',(e)=>{if(e.target.closest('[data-idle-farm-exit]'))chooseIdleBattleMode('progress');});
  ...
  document.getElementById('idle-launch-reward')?.addEventListener('click',(e)=>{const b=e.target.closest('[data-idle-launch-claim]');if(b&&!b.disabled)claimIdleLaunchReward();});
  ...
  const handleIdleAdvisorClick=(e)=>{const button=e.target.closest('[data-advisor-tab]');if(!button)return;const command=button.dataset.advisorCommand;if(command==='claim-all'){idleShowPanel('activities');return claimAllIdle();}if(command==='rank-up'){idleShowPanel('progression');return advanceIdleRank();}if(command==='battle-progress'){idleShowPanel('home');return chooseIdleBattleMode('progress');}idleShowPanel(button.dataset.advisorTab||'home');};
  document.getElementById('idle-advisor')?.addEventListener('click',handleIdleAdvisorClick);
  document.getElementById('idle-focus-objective')?.addEventListener('click',handleIdleAdvisorClick);
  document.getElementById('idle-team-compare')?.addEventListener('click',(e)=>{const button=e.target.closest('[data-team-compare-slot]');if(!button)return;const slot=idleState?.slots?.find((entry)=>entry.index===Number(button.dataset.teamCompareSlot));if(slot?.character)openIdleCharacterSheet(slot.character);});
  document.getElementById('idle-competitive-tabs')?.addEventListener('click',(e)=>{const button=e.target.closest('[data-competitive-tab]');if(!button)return;idleCompetitiveTab=button.dataset.competitiveTab;localStorage.setItem('idle-competitive-tab',idleCompetitiveTab);renderIdleCompetitiveCenter(idleState);});
  const competitiveContent=document.getElementById('idle-competitive-content');
  competitiveContent?.addEventListener('change',(e)=>{if(e.target.matches('[data-comp-sim-formation]')){idleCompetitiveSimulation.formation=e.target.value;renderIdleCompetitiveCenter(idleState);}});
  competitiveContent?.addEventListener('click',(e)=>{const goto=e.target.closest('[data-comp-goto]');if(goto){const slot=goto.dataset.compSlot;if(slot!=null)return openIdleEquipmentForSlot(Number(slot));idleShowPanel(goto.dataset.compGoto);return;}const save=e.target.closest('[data-comp-preset-save]');if(save)return saveIdlePreset(Number(save.dataset.compPresetSave));const load=e.target.closest('[data-comp-preset-load]');if(load)return loadIdlePreset(load.dataset.compPresetLoad);const ranking=e.target.closest('[data-comp-ranking]');if(ranking)return openIdleRanking(ranking.dataset.compRanking);const sim=e.target.closest('[data-comp-sim]');if(sim){const [key,raw]=sim.dataset.compSim.split(':');idleCompetitiveSimulation[key]=Math.max(0,Math.min(key==='levels'?100:15,Number(idleCompetitiveSimulation[key]||0)+Number(raw)));renderIdleCompetitiveCenter(idleState);return;}if(e.target.closest('[data-comp-select-dominated]')){const dominated=idleCompetitiveDominatedItems(idleState);idleSelectedItems=new Set(dominated.map((item)=>item.id));idleShowPanel('equipment');requestAnimationFrame(()=>renderIdleInventory(idleState));return;}if(e.target.closest('[data-comp-share-build]'))shareIdleCompetitiveBuild();});
