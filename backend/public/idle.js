...
function renderIdleInventory(state){
  ...
  const loadouts=document.getElementById('idle-loadouts');if(loadouts)loadouts.innerHTML=active.length?active.map((slot)=>{const equipped=inventory.items.filter((x)=>x.equippedSlotIndex===slot.index);const total=equipped.reduce((sum,item)=>sum+(item.effectiveBonus||item.bonus||0),0);const sets=idleRuneSetSummary(equipped);return `<article class="idle-loadout ${sets?'complete':''}"><header><span class="idle-loadout-portrait" role="img" aria-label="Portrait de ${escapeHtml(slot.character.name)}" ${slot.character.imageUrl?`style="background-image:url('${escapeHtml(slot.character.imageUrl)}')"`:''}></span><span><small>${escapeHtml(idleRoleFor(slot.character).name)} · ${equipped.length}/6 runes</small><b>${escapeHtml(slot.character.name)}</b><em><i class="fas fa-layer-group"></i> ${escapeHtml(sets||'Aucun set actif')}</em></span><strong title="Bonus principal cumulé">Stats +${Math.round(total*100)}%</strong></header><div class="idle-loadout-items">${IDLE_RUNE_KINDS.map((kind)=>{const meta=IDLE_ITEM_META[kind];const item=equipped.find((x)=>x.kind===kind);return item?`<button type="button" data-loadout-item="${escapeHtml(item.id)}" class="r-${item.rarity}" title="Retrouver ${escapeHtml(item.name)} dans l’inventaire">${idleItemArt(item,'mini')}<span><small>${meta.label} · ${escapeHtml(item.setName||'Énergie')}</small><b>+${item.enhancementLevel||0} · ${escapeHtml(item.name)}</b><em>${escapeHtml(idleItemEffect(item))}</em></span><strong>${Math.round((item.effectiveBonus||item.bonus||0)*100)}%</strong><i class="fas fa-chevron-down"></i></button>`:`<span class="idle-loadout-empty"><i class="fas ${meta.icon}"></i><span><small>${meta.label}</small><b>Vide</b></span></span>`;}).join('')}</div></article>`;}).join(''):'<p class="hint">Assigne un personnage à ton équipe pour pouvoir l’équiper.</p>';
  ...
  grid.innerHTML=items.length?items.map((item)=>{
    ...
    const [primaryAffix,...secondaryAffixes]=item.affixesDetailed||[];
    ...
    return `<article class="idle-item-card r-${item.rarity} ${equipped?'equipped':''} ${selected?'selected':''}" data-item-id="${item.id}"><header>${idleItemArt(item)}<div><small>${escapeHtml(idleRarityLabel(item.rarity))} · ${meta.label} · Set ${escapeHtml(item.setName||'Énergie')}</small><b>${escapeHtml(item.name)}</b></div><span class="idle-item-card-actions"><button data-item-select="${item.id}" title="${selectable?(selected?'Retirer de la sélection':'Sélectionner pour recycler'):'Verrouillé ou équipé'}" ${selectable?'':'disabled'}><i class="fas ${selected?'fa-square-check':'fa-square'}"></i></button><button data-item-lock="${item.id}" title="${item.locked?'Déverrouiller':'Verrouiller'}"><i class="fas ${item.locked?'fa-lock':'fa-lock-open'}"></i></button></span></header><div class="idle-item-stats"><span><small>AMÉLIORATION</small><b>+${item.enhancementLevel||0} / +15</b><em>${escapeHtml(idleItemEffect(item))}</em></span><span><small>${escapeHtml(primaryAffix?.label||item.effectLabel)}</small><b>+${Math.round((primaryAffix?.value??item.effectValue)*100)}%</b><em>${escapeHtml(item.setDescription||primaryAffix?.description||'Bonus de set')}</em></span>${secondaryAffixes.map((a)=>`<span class="idle-item-affix-extra"><small>${escapeHtml(a.label)}</small><b>+${Math.round(a.value*100)}%</b><em>${escapeHtml(a.description||'')}</em></span>`).join('')}<span class="idle-item-affix-extra"><small>SOUS-STATS DE RUNE</small><b>${escapeHtml(idleRuneSubStats(item))}</b></span></div>${equipped?`<p class="idle-item-equipped"><i class="fas fa-user-shield"></i> Équipée sur <b>${escapeHtml(item.equippedCharacter||'un héros')}</b>${item.equippedResting?' <em>(au repos, hors de l’équipe active)</em>':''}</p>`:active.length?`<label class="idle-item-target">Équiper sur <select data-item-target>${active.map((slot)=>`<option value="${slot.index}" ${slot.index===defaultSlot?'selected':''}>${escapeHtml(slot.character.name)}</option>`).join('')}</select></label><div data-item-comparison>${idleItemComparison(item,defaultSlot)}</div>`:'<p class="idle-item-equipped">Aucun héros actif</p>'}<footer>${equipped?`<button class="btn-secondary" data-item-enhance="${item.id}" data-item-enhance-amount="1" ${maxed||idleState.essence<item.enhanceCost?'disabled':''}><i class="fas fa-arrow-trend-up"></i> ${maxed?'Rune +15':`Améliorer +${(item.enhancementLevel||0)+1} · ${idleFormatNumber(item.enhanceCost)}`}</button><button class="btn-secondary" data-item-unequip="${item.id}"><i class="fas fa-arrow-right-from-bracket"></i> Retirer</button>`:active.length?`<button class="btn-primary" data-item-equip="${item.id}"><i class="fas fa-shield-halved"></i> Équiper</button>`:''}<button class="btn-secondary danger" data-item-salvage="${item.id}" ${selectable?'':'disabled'}><i class="fas fa-recycle"></i> ${idleFormatNumber(item.salvageValue)}</button></footer></article>`;
