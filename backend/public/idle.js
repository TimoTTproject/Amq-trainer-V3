...
function renderIdleCollection(codex) {
  ...
  if (summary) summary.innerHTML = `<span><i class="fas fa-user-check"></i><b>${idleFormatNumber(codex?.discovered || 0)}</b><small>héros recrutés</small></span><span><i class="fas fa-book-open"></i><b>${idleFormatNumber(codex?.catalogTotal || 0)}</b><small>personnages au total</small></span><span><i class="fas fa-layer-group"></i><b>${collection.filter((s) => s.complete).length}</b><small>licences complètes</small></span>`;
...
const IDLE_ITEM_META=Object.fromEntries(Array.from({length:6},(_,index)=>[`rune${index+1}`,{label:`Rune ${index+1}`,icon:'fa-diamond'}]));
...
function idleRuneSetSummary(items){const counts=new Map();for(const item of items)counts.set(item.setName||item.setKey,(counts.get(item.setName||item.setKey)||0)+1);return [...counts.entries()].map(([name,count])=>`${name} ${count}`).join(' · ');}
...
function renderIdleInventory(state){
  ...
  const summary=document.getElementById('idle-item-collection-stats');if(summary){const s=inventory.summary||{};summary.innerHTML=`<span><i class="fas fa-layer-group"></i><b>${s.worlds||0}</b><small>Sets trouvés</small></span><span><i class="fas fa-wand-sparkles"></i><b>${s.effects||0}</b><small>Stats différentes</small></span><span><i class="fas fa-circle-check"></i><b>${s.completeFamilies||0}</b><small>Sets activables</small></span><span><i class="fas fa-user-shield"></i><b>${s.equipped||0}</b><small>Runes équipées</small></span>`;}
  const families=document.getElementById('idle-item-families');if(families)families.innerHTML=(inventory.families||[]).map((family)=>`<span class="${family.complete?'complete':''}"><b>${escapeHtml(family.world)}</b><em>${family.count}/${family.required} runes</em><small>${escapeHtml(family.description||'Bonus de set')}</small></span>`).join('');
  ...
  const loadouts=document.getElementById('idle-loadouts');if(loadouts)loadouts.innerHTML=active.length?active.map((slot)=>{const equipped=inventory.items.filter((x)=>x.equippedSlotIndex===slot.index);const total=equipped.reduce((sum,item)=>sum+(item.effectiveBonus||item.bonus||0),0);const sets=idleRuneSetSummary(equipped);const selected=slot.index===idleEquipmentTargetSlot;return `<article class="idle-loadout ${sets?'complete':''} ${selected?'selected':''}"><header><span class="idle-loadout-portrait" role="img" aria-label="Portrait de ${escapeHtml(slot.character.name)}" ${slot.character.imageUrl?`style="background-image:url('${escapeHtml(slot.character.imageUrl)}')"`:''}></span><span><small>${escapeHtml(idleRoleFor(slot.character).name)} · ${equipped.length}/6 runes</small><b>${escapeHtml(slot.character.name)}</b><em><i class="fas fa-layer-group"></i> ${escapeHtml(sets||'Aucun set actif')}</em></span><strong title="Puissance cumulée des runes">+${Math.round(total*100)}%</strong><button type="button" class="idle-loadout-select" data-loadout-target="${slot.index}"><i class="fas ${selected?'fa-circle-check':'fa-crosshairs'}"></i> ${selected?'Héros sélectionné':'Équiper ce héros'}</button></header><div class="idle-loadout-items">${IDLE_RUNE_KINDS.map((kind)=>{const meta=IDLE_ITEM_META[kind];const item=equipped.find((x)=>x.kind===kind);return item?`<button type="button" data-loadout-item="${escapeHtml(item.id)}" class="r-${item.rarity}" title="Retrouver ${escapeHtml(item.name)} dans l’inventaire">${idleItemArt(item,'mini')}<span><small>${meta.label} · Set ${escapeHtml(item.setName||'Énergie')}</small><b>+${item.enhancementLevel||0} · ${escapeHtml(item.name)}</b><em>${escapeHtml(idleItemEffect(item))}</em></span><strong>${Math.round((item.effectiveBonus||item.bonus||0)*100)}%</strong><i class="fas fa-chevron-down"></i></button>`:`<span class="idle-loadout-empty"><i class="fas ${meta.icon}"></i><span><small>${meta.label}</small><b>Emplacement vide</b></span></span>`;}).join('')}</div></article>`;}).join(''):'<p class="hint">Assigne un personnage à ton équipe pour pouvoir l’équiper.</p>';
...
function showIdleBossReward(reward) {
  ...
  const names = Object.fromEntries(IDLE_RUNE_KINDS.map((kind,index)=>[kind,`Rune ${index+1}`]));
  const loot=reward.loot;
  body.innerHTML=`<div class="idle-boss-reward-main"><i class="fas fa-trophy"></i><span><small>COFFRE ${reward.tier}</small><b>Butin du gardien</b></span></div><div class="idle-boss-reward-grid"><span><i class="fas fa-bolt"></i><b>+${idleFormatNumber(reward.reward)}</b><small>Essence</small></span><span><i class="fas fa-ticket"></i><b>+${reward.seals}</b><small>Sceau${reward.seals>1?'x':''}</small></span></div>${loot?`<div class="idle-boss-loot ${escapeHtml(loot.rarity)}">${idleItemArt(loot,'reveal')}<div><small>${escapeHtml(loot.rarity.toUpperCase())} · SET ${escapeHtml(loot.setKey||'energy').toUpperCase()}</small><b>${escapeHtml(loot.name||names[loot.kind])}</b><span>${loot.stored?`Rune ajoutée · ${escapeHtml(names[loot.kind]||'Rune')} · +0`:`Inventaire plein · convertie en +${idleFormatNumber(loot.salvage||0)} Essence`}</span></div><em>${loot.stored?'NOUVELLE':'RECYCLÉE'}</em></div>${loot.stored?'<button class="btn-secondary idle-boss-open-items" data-open-equipment><i class="fas fa-diamond"></i> Voir et équiper la rune</button>':''}`:''}`;
  modal.classList.remove('hidden');
}
...
function renderIdleRift(rift){
  ...
  if(!rift.unlocked){
    box.innerHTML=`<div class="idle-locked-teaser"><i class="fas fa-lock"></i><div><small>DÉFI HEBDOMADAIRE</small><b>Faille dimensionnelle</b><span>Débloqué au niveau ${rift.unlockLevel}</span></div></div>`;
    return;
  }
...
function renderIdlePrestige(dojo) {
  ...
  if(paths){
    // Le choix de voie ne sert qu'APRÈS un premier Prestige (rien à choisir
    // avant) — un joueur qui n'a jamais prestigé voyait déjà 4 boutons
    // détaillés désactivés sans pouvoir agir dessus. Un aperçu verrouillé
    // évite ce bruit tant que la fonctionnalité n'est pas pertinente.
    paths.classList.toggle('locked',dojo.prestige.level<1);
    paths.innerHTML=dojo.prestige.level<1
      ? `<div class="idle-locked-teaser"><i class="fas fa-lock"></i><div><small>STRATÉGIE DE PRESTIGE</small><b>Voies de Prestige</b><span>Débloqué après ton premier Prestige</span></div></div>`
      : (dojo.prestige.paths||[]).map((p)=>`<button data-prestige-path="${p.key}" class="${p.selected?'active':''}"><i class="fas ${p.key==='fist'?'fa-hand-fist':p.key==='army'?'fa-users':p.key==='time'?'fa-clock':'fa-scale-balanced'}"></i><b>${escapeHtml(p.name)}</b><small>Équipe ×${p.prod.toFixed(2)} · Clic ×${p.click.toFixed(2)}</small></button>`).join('');
  }
...
function idleSlotHTML(slot) {
  ...
  const equipment=c.equipments.map((e)=>{const meta=IDLE_ITEM_META[e.kind]||IDLE_ITEM_META.rune1;return e.empty?`<span class="empty" title="${meta.label} non équipée"><i class="fas ${meta.icon}"></i><div><small>${meta.label}</small><b>Vide</b></div></span>`:`<button class="r-${e.rarity}" data-action="enhance-equipment" data-item-enhance-id="${e.id}" title="${e.enhancementLevel>=15?'Rune au maximum':`Améliorer ${meta.label} · coût ${idleFormatNumber(e.enhanceCost)}`}"><i class="fas ${meta.icon}"></i><span><small>${meta.label} · ${escapeHtml(e.setName||'Énergie')}</small><b>+${e.enhancementLevel||0}</b><em>${escapeHtml(idleItemEffect(e))}</em></span></button>`;}).join('');
...
async function enhanceIdleEquipment(itemId,amount=1){try{const state=await api('/api/idle/equipment/enhance',{method:'POST',body:JSON.stringify({itemId,amount})});idleSpawnFloat('RUNE AMÉLIORÉE','xp');renderIdleState(state);requestAnimationFrame(()=>{const card=Array.from(document.querySelectorAll('#idle-inventory-grid [data-item-id]')).find((node)=>node.dataset.itemId===String(itemId));card?.scrollIntoView({block:'center'});card?.classList.add('idle-item-focus');setTimeout(()=>card?.classList.remove('idle-item-focus'),1000);});}catch(e){idleNotify(e.message,'error');}}
