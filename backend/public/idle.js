...
function renderIdleInventory(state){
  ...
  const families=document.getElementById('idle-item-families');if(families)families.innerHTML=(inventory.families||[]).map((family)=>`<span class="${family.complete?'complete':''}"><b>Set ${escapeHtml(family.world)}</b><em>${family.count} possédé${family.count>1?'s':''} · ${family.required} requis sur un héros</em><small>${family.complete?'<i class="fas fa-circle-check"></i> ASSEMBLABLE':'Encore '+Math.max(0,family.required-family.count)+' à trouver'} · ${escapeHtml(family.description||'Bonus de set')}</small></span>`).join('');
