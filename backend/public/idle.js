...
function renderIdleQuickBuy(state) {
  ...
  box.innerHTML = active.map((slot) => {
    ...
    return `<article class="idle-quick-hero r-${escapeHtml(c.rarity)} ${c.awakened ? 'is-awakened' : ''}">
      ...
      <span class="idle-quick-info"><b>${escapeHtml(c.name)}</b><small>Nv. ${idleFormatNumber(c.level)} · +${idleFormatNumber(c.rate)}/s</small><em class="idle-quick-milestone" title="Chaque palier double la production de ce héros"><i style="--progress:${milestoneProgress}%"></i><span><i class="fas fa-flag-checkered"></i> ${milestone}</span></em></span>
...
function openIdleCharacterSheet(character){
  ...
  const passive=c.passive||({rare:'Endurance · +5% de production personnelle au niveau 10',epic:'Aura · +2% de production à toute l’équipe au niveau 10',legendary:'Domination · +4% de production à toute l’équipe au niveau 10',mythic:'Transcendance · +6% de production à toute l’équipe au niveau 10'}[c.rarity]||'Bonus débloqué au niveau 10');
...
function renderIdleInventory(state){
  ...
  grid.innerHTML=items.length?items.map((item)=>{
    const meta=IDLE_ITEM_META[item.kind]||IDLE_ITEM_META.weapon;const equipped=item.equipped;const selectable=!item.locked&&!equipped;const selected=idleSelectedItems.has(item.id);const defaultSlot=equipped?item.equippedSlotIndex:active[0]?.index;
    ...
    return `<article class="idle-item-card r-${item.rarity} ${equipped?'equipped':''} ${selected?'selected':''}" data-item-id="${item.id}"><header>${idleItemArt(item)}<div><small>${escapeHtml(idleRarityLabel(item.rarity))} · ${meta.label} · ${escapeHtml(item.sourceWorld)}</small><b>${escapeHtml(item.name)}</b></div><span class="idle-item-card-actions"><button data-item-select="${item.id}" title="${selectable?(selected?'Retirer de la sélection':'Sélectionner pour recycler'):'Verrouillé ou équipé'}" ${selectable?'':'disabled'}><i class="fas ${selected?'fa-square-check':'fa-square'}"></i></button><button data-item-lock="${item.id}" title="${item.locked?'Déverrouiller':'Verrouiller'}"><i class="fas ${item.locked?'fa-lock':'fa-lock-open'}"></i></button></span></header><div class="idle-item-stats"><span><small>NIVEAU D’OBJET</small><b>${idleFormatNumber(item.powerLevel)}</b><em>+${Math.round(item.bonus*100)}% de base</em></span><span><small>${escapeHtml(primaryAffix?.label||item.effectLabel)}</small><b>+${Math.round((primaryAffix?.value??item.effectValue)*100)}%</b><em>${escapeHtml(primaryAffix?.description||item.effectDescription||'')}</em></span>${secondaryAffixes.map((a)=>`<span class="idle-item-affix-extra"><small>${escapeHtml(a.label)}</small><b>+${Math.round(a.value*100)}%</b><em>${escapeHtml(a.description||'')}</em></span>`).join('')}</div>${equipped?`<p class="idle-item-equipped"><i class="fas fa-user-shield"></i> Équipé sur <b>${escapeHtml(item.equippedCharacter||'un héros')}</b>${item.equippedResting?' <em>(au repos, hors de l’équipe active)</em>':''}</p>`:active.length?`<label class="idle-item-target">Comparer avec <select data-item-target>${active.map((slot)=>`<option value="${slot.index}" ${slot.index===defaultSlot?'selected':''}>${escapeHtml(slot.character.name)}</option>`).join('')}</select></label><div data-item-comparison>${idleItemComparison(item,defaultSlot)}</div>`:'<p class="idle-item-equipped">Aucun héros actif</p>'}<footer>${equipped?`<div class="idle-item-enhance-group"><button class="btn-secondary" data-item-enhance="${item.id}" data-item-enhance-amount="1" ${idleState.essence<item.enhanceCost?'disabled':''}><i class="fas fa-arrow-trend-up"></i> Améliorer · ${idleFormatNumber(item.enhanceCost)}</button>${['10','100'].map((n)=>{const bulk=item.enhanceBulkCosts?.[n];return bulk?`<button class="btn-secondary" data-item-enhance="${item.id}" data-item-enhance-amount="${n}" ${idleState.essence<bulk.cost?'disabled':''}>×${n} · ${idleFormatNumber(bulk.cost)}</button>`:'';}).join('')}</div><button class="btn-secondary" data-item-unequip="${item.id}"><i class="fas fa-arrow-right-from-bracket"></i> Retirer</button>`:active.length?`<button class="btn-primary" data-item-equip="${item.id}"><i class="fas fa-shield-halved"></i> Équiper</button>`:''}<button class="btn-secondary danger" data-item-salvage="${item.id}" ${selectable?'':'disabled'}><i class="fas fa-recycle"></i> ${idleFormatNumber(item.salvageValue)}</button></footer></article>`;
...
async function enhanceIdleEquipment(itemId,amount=1){try{const state=await api('/api/idle/equipment/enhance',{method:'POST',body:JSON.stringify({itemId,amount})});idleSpawnFloat(amount===1?'ÉQUIPEMENT +1%':`ÉQUIPEMENT ×${amount}`,'xp');renderIdleState(state);}catch(e){idleNotify(e.message,'error');}}
