...
async function loadAdminIdleBalance(){
  ...
  try{const data=await api('/api/idle/diagnostics/balance');const number=(value)=>Number(value||0).toLocaleString('fr-FR');
    ...
    const choices=data.builds?.choices||[];box.insertAdjacentHTML('beforeend',`<div class="admin-idle-split"><section><h4>Pouvoirs roguelike choisis</h4>${choices.slice(0,8).map((choice)=>`<span class="admin-idle-wall"><b>${escapeHtml(choice.name)}</b><em>${number(choice.count)} choix</em><strong>${choice.percent}%</strong></span>`).join('')||'<p class="hint">Pas encore assez de choix enregistrés.</p>'}</section><section><h4>Engagement endgame</h4><div class="admin-idle-rift"><span><small>EXPÉDITIONS LANCÉES</small><b>${number((data.engagement?.expeditions||[]).find((event)=>event.event==='expedition_start')?.count)}</b></span><span><small>EXPÉDITIONS RÉCLAMÉES</small><b>${number((data.engagement?.expeditions||[]).find((event)=>event.event==='expedition_claim')?.count)}</b></span><span><small>AUTO-PROTECTIONS</small><b>${number(data.engagement?.equipmentAutoLocks)}</b></span><span><small>MEULAGES AMÉLIORÉS</small><b>${number(data.engagement?.rerollUpgradeRate)}%</b></span></div></section></div>`);
